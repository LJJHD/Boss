...
- (void)initialization {
    ...
    _pointImage = [UIImage imageNamed:@"circle_point3" inBundle:resourcesBundle compatibleWithTraitCollection:nil];
}
...
//        UIFont *font = [UIFont systemFontOfSize:0.15*CircleSelfWidth];
        ...
        //构建属性集合
        NSDictionary *attributes = @{NSFontAttributeName:font, NSParagraphStyleAttributeName:textStyle, NSForegroundColorAttributeName:DEF_COLOR_965800};
        //获得size
        ...
        //段落格式
        NSMutableParagraphStyle *percentTextStyle = [[NSMutableParagraphStyle defaultParagraphStyle] mutableCopy];
        percentTextStyle.lineBreakMode = NSLineBreakByWordWrapping;
        percentTextStyle.alignment = NSTextAlignmentCenter;//水平居中
        //字体
        //        UIFont *font = [UIFont systemFontOfSize:0.15*CircleSelfWidth];
        UIFont *percentFont = [UIFont systemFontOfSize:24];
        //构建属性集合
        NSDictionary *percentAttributes = @{NSFontAttributeName:percentFont, NSParagraphStyleAttributeName:percentTextStyle, NSForegroundColorAttributeName:DEF_COLOR_965800};
        //获得size
        CGSize percentStringSize = [percentText sizeWithAttributes:percentAttributes];
        //垂直居中
        CGRect rect = CGRectMake((CircleSelfWidth-stringSize.width)/2.0 + stringSize.width, (CircleSelfHeight - stringSize.height)/2.0 + stringSize.height - percentStringSize.height - 10, percentStringSize.width, percentStringSize.height);
        [percentText drawInRect:rect withAttributes:percentAttributes];
        
        //画文字
        testingString = @"正在体检";
        //段落格式
        NSMutableParagraphStyle *testTextStyle = [[NSMutableParagraphStyle defaultParagraphStyle] mutableCopy];
        testTextStyle.lineBreakMode = NSLineBreakByWordWrapping;
        testTextStyle.alignment = NSTextAlignmentCenter;//水平居中
        //字体
        //        UIFont *font = [UIFont systemFontOfSize:0.15*CircleSelfWidth];
        UIFont *testFont = [UIFont systemFontOfSize:15];
        //构建属性集合
        NSDictionary *testAttributes = @{NSFontAttributeName:testFont, NSParagraphStyleAttributeName:testTextStyle, NSForegroundColorAttributeName:DEF_COLOR_CDA265};
        //获得size
        CGSize testStringSize = [testingString sizeWithAttributes:testAttributes];
        //垂直居中
        CGRect testR = CGRectMake((CircleSelfWidth-testStringSize.width)/2.0, (CircleSelfHeight - stringSize.height)/2.0 + stringSize.height + 10,testStringSize.width, testStringSize.height);
        [testingString drawInRect:testR withAttributes:testAttributes];
    }
