...
- (void)recompute {
    ...
    //同理
    CGFloat minimal = (CGRectGetWidth(self.bounds) < CGRectGetHeight(self.bounds)) ? CGRectGetWidth(self.bounds) : CGRectGetHeight(self.bounds);
    //ljj修改 之前为 outerCircleRadius = 2  innerCircleRadius = 6
    if (self.innerRadius > 0 && self.outerRadius > 0) {
        
        self.outerCircleRadius = minimal / self.outerRadius;
        self.innerCircleRadius = minimal / self.innerRadius;
    }else{
