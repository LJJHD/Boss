...
- (void)setYLabels:(NSArray *)yLabels {
    ...
    if (_showLabel) {
        yLabelHeight = (_chartCavanHeight - _chartInnerMarginTop - _chartInnerMarginBottom) / ([yLabels count] - 1);
    } else {
...
- (void)setXLabels:(NSArray *)xLabels {
    ...
    if (_showLabel) {
        xLabelWidth = (_chartCavanWidth - _chartInnerMarginLeft - _chartInnerMarginRight) / ([xLabels count] - 1);
    } else {
...
- (void)setXLabels:(NSArray *)xLabels withWidth:(CGFloat)width {
    ...
    if (_showLabel) {
        for (NSUInteger index = 0; index < xLabels.count; index++) {
            ...
            NSInteger x = (NSInteger) (index * _xLabelWidth + _chartMarginLeft + _chartInnerMarginLeft - _xLabelWidth / 2);
            NSInteger y = (NSInteger) (_chartMarginTop + _chartCavanHeight);
...
//            distance /= hypot(p2.x - p1.x, p1.y - p2.y);
            ...
            float distance = [self CalDis:p1.x :p1.y :p2.x :p2.y :touchPoint.x :touchPoint.y];
  ...
  andProgressLinePathsColors:_progressLinePathsColors];
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        if (chartData.inflexionPointColor) {
            pointLayer.fillColor = [[chartData.inflexionPointColor
                    colorWithAlphaComponent:chartData.alpha] CGColor];
        } else {
            pointLayer.fillColor = [PNGreen CGColor];
        }
...
andProgressLinePathsColors:(NSMutableArray *)progressLinePathsColors {
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            float x = i * _xLabelWidth + _chartMarginLeft + _chartInnerMarginLeft;
            float y = [self yValuePositionInLineChart:yValue];
        ...
        if (lineStartEndPointsArray.count && linePointsArray.count) {
...
- (void)removeLayers {
    ...
    for (NSArray<CALayer *> *layers in self.gradeStringPaths) {
        for (CALayer *layer in layers) {
            [layer removeFromSuperlayer];
        }
    }
...
-(void) resetCavanHeight {
    ...
    if (!_showLabel) {
        ...
        _xLabelWidth = (_chartCavanWidth - _chartInnerMarginLeft - _chartInnerMarginRight / [_xLabels count]);
    }
}
...
//        pointLayer.strokeColor = [[chartData.color colorWithAlphaComponent:chartData.alpha] CGColor];
        ...
        pointLayer.fillColor = [[chartData.color colorWithAlphaComponent:chartData.alpha] CGColor];
...
- (void)drawRect:(CGRect)rect {
    if (self.isShowCoordinateAxis) {
        ...
        CGFloat yAxisHeight = CGRectGetHeight(rect) - (_chartMarginBottom + _chartMarginTop);

        // draw coordinate axis
        CGContextMoveToPoint(ctx, _chartMarginLeft, _chartMarginTop);
        CGContextAddLineToPoint(ctx, _chartMarginLeft, yAxisHeight + _chartMarginTop);
        CGContextAddLineToPoint(ctx, xAxisWidth + _chartMarginLeft, yAxisHeight + _chartMarginTop);
        CGContextStrokePath(ctx);

        // draw y axis arrow
        CGContextMoveToPoint(ctx, _chartMarginLeft - 3, _chartMarginTop + 6);
        CGContextAddLineToPoint(ctx, _chartMarginLeft, _chartMarginTop);
        CGContextAddLineToPoint(ctx, _chartMarginLeft + 3, _chartMarginTop + 6);
        CGContextStrokePath(ctx);

        // draw x axis arrow
        CGContextMoveToPoint(ctx, xAxisWidth + _chartMarginLeft - 6, yAxisHeight + _chartMarginTop - 3);
        CGContextAddLineToPoint(ctx, xAxisWidth + _chartMarginLeft, yAxisHeight + _chartMarginTop);
        CGContextAddLineToPoint(ctx, xAxisWidth + _chartMarginLeft - 6, yAxisHeight + _chartMarginTop + 3);
        CGContextStrokePath(ctx);
        ...
        if (self.showLabel) {
            ...
            CGPoint point;
            CGFloat xStepHeight = (_chartCavanWidth - _chartInnerMarginLeft - _chartInnerMarginRight) / ([self.xLabels count] - 1);
            if (self.showXMark) {
                for (NSUInteger i = 0; i < [self.xLabels count]; i++) {
                    point = CGPointMake(_chartMarginLeft + _chartInnerMarginLeft + (i * xStepHeight), CGRectGetHeight(rect) - _chartMarginBottom);
                    CGContextMoveToPoint(ctx, point.x, point.y);
                    CGContextAddLineToPoint(ctx, point.x, point.y - 2);
                    CGContextStrokePath(ctx);
                }
            }

            // draw y axis separator
            CGFloat yStepHeight = (_chartCavanHeight - _chartInnerMarginBottom - _chartInnerMarginTop) / _yLabelNum;
            if (self.showYMark) {
                for (NSUInteger i = 0; i < [self.yLabels count]; i++) {
                    point = CGPointMake(_chartMarginLeft, CGRectGetHeight(rect) - _chartMarginBottom - _chartInnerMarginBottom - i * yStepHeight);
                    CGContextMoveToPoint(ctx, point.x, point.y);
        ...
        if ([self.yUnit length]) {
            ...
            CGRect drawRect = CGRectMake(_chartMarginLeft - 15, _chartMarginTop - height - 4, 30.f, height);
        ...
        if ([self.xUnit length]) {
            ...
            CGRect drawRect = CGRectMake(CGRectGetWidth(rect) - _chartMarginRight - 10, _chartMarginBottom + _chartCavanHeight + 4, 25.f, height);
    ...
    if (self.showYGridLines) {
        ...
        for (NSUInteger i = (_chartInnerMarginBottom < 0.01); i <= _yLabelNum; i++) {
            point = CGPointMake(_chartMarginLeft, CGRectGetHeight(rect) - _chartMarginBottom - _chartInnerMarginBottom - i * yStepHeight);
            CGContextMoveToPoint(ctx, point.x, point.y);
...
- (CGFloat)yValuePositionInLineChart:(CGFloat)y {
    ...
    return _chartCavanHeight + _chartMarginTop - _chartInnerMarginBottom - (innerGrade * (_chartCavanHeight - _chartInnerMarginBottom - _chartInnerMarginTop));
}
 ...
 * @param p1 default
 * @param p2 default
 * @param rangeColors default
 * @param defaultColor default
 * @return default
 */
...
//    [textLayer setCornerRadius:(CGFloat) (textLayer.fontSize / 8.0)];
    ...
    if (pointCenter.x - textWidth/2 <= _chartMarginLeft) {
        [textLayer setFrame:CGRectMake(0, 0, textWidth - 2 * fabs(pointCenter.x - textWidth/2 - _chartMarginLeft), textHeight)];
        [textLayer setPosition:CGPointMake(pointCenter.x + 2, textStartPosY)];
    }
