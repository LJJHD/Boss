...
-(void)layoutSubviews{
    ...
    for (int i = 0; i < self.nodeArray.count; ++i) {
        ...
        CGFloat width = 80;
        CGFloat height = 80;
        CGFloat padding = (self.bounds.size.width - width * 3) / 4;
        int row = i % 3;
        int column = i / 3;
        CGRect frame = CGRectMake(padding + row * (width + padding), column * (height + padding), width, height);
...
-(void)cleanNodes
{
    ...
    self.polygonalLineLayer.strokeColor = DEF_COLOR_CDA265.CGColor;
    self.polygonalLineLayer.path = self.polygonalLinePath.CGPath;
}
